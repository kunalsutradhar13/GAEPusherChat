...
public class SendMessageServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// [{"message": "my-message", "socket_id": "1232.24", "channel":
		// "presence-my-channel"}]
		...
		// Create a message including the user email prefix to display in the chat
		// window
		StringBuilder strMessage = new StringBuilder();
		String taggedMessage = "<strong>&lt;" + displayName + "&gt;</strong> " + message + "<br>";
		strMessage.append(taggedMessage);
		String taggedMessage1 = "<strong>&lt;" + "admin" + "&gt;</strong> " + "Something Rubbish ";
		strMessage.append(taggedMessage1);

		// taggedMessage1.concat(taggedMessage);
		Map<String, String> messageData = new HashMap<>();
		messageData.put("message", strMessage.toString());
		...
		Result result = PusherService.getDefaultInstance().trigger(channelId, "new_message", // name of event
				messageData, socketId); // (Optional) Use client socket_id to exclude the sender from receiving the
										// message
